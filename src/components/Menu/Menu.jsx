...
export default function Menu({ handleClickLogout }) {
  const [isOpen, setIsOpen] = React.useState(false);
  const { fullName, createdAt: created, isAuth } = useSelector((state) => state.user);
  ...
  const { handleClickOpenForm } = React.useContext(formContextProvider);

  const openModalWindow = (name) => {
    handleClickOpenForm(name);
    setIsOpen(false);
  };

  return (
    <div className={styles.menu}>
      <div className={styles.menu__fixed + ' ' + (isOpen ? styles.menu__open : null)}>
        {isOpen && (
